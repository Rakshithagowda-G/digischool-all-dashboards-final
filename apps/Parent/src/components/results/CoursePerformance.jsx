...
function CoursePerformance({ tests = [] }) {
  ...
  // Update expanded state when tests arrive
  useEffect(() => {
    if (safeTests.length > 0) {
      const initialExpanded = safeTests.reduce(
        (acc, test) => ({ ...acc, [test.id]: true }),
        {}
      );
  ...
  const renderTestCard = (test) => {
    const testItems = Array.isArray(test.testData) ? test.testData : [];
    const totalMarks = testItems.reduce((sum, item) => sum + (item.maxMarks || 0), 0);
    const marksObtained = testItems.reduce((sum, item) => sum + (item.result || 0), 0);
        ...
        <Box>
          ...
          >
            <Box>
              ...
              <Typography sx={{ color: isDarkMode ? "#fff" : "#1c204b", fontWeight: 700, fontFamily: "Poppins, sans-serif" }}>{test.rank || "N/A"}</Typography>
            </Box>
            <Box>
              <Typography variant="caption" sx={{ color: isDarkMode ? "rgba(255,255,255,0.7)" : "rgba(28,32,75,0.7)", fontFamily: "Poppins, sans-serif" }}>
                Total
  ...
  if (safeTests.length === 0) {
    return (
      <Box display="flex" flexDirection="column" alignItems="center" justifyContent="center" p={5} sx={{ background: isDarkMode ? "rgba(255,255,255,0.05)" : "#fff", borderRadius: 4 }}>
        <Info size={48} color="gray" />
        <Typography variant="h6" sx={{ mt: 2, color: "gray", fontStyle: 'italic' }}>No test records available</Typography>
      </Box>
    );
  }
