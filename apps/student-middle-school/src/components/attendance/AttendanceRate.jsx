...
export default function AttendanceRate({ monthlyRate = {} }) {
  const { year = new Date().getFullYear(), monthlyRates = [] } = monthlyRate;

  // Calculate overall attendance percentage from monthly rates
  const overallPercentage = monthlyRates.length > 0
    ? Math.round(monthlyRates.reduce((sum, m) => sum + (m.percentage || 0), 0) / monthlyRates.length)
    : 0;

  const months = defaultMonths.map((defaultMonth) => {
    const backendData = monthlyRates?.find(m =>
      m.month.toLowerCase() === defaultMonth.name.toLowerCase()
    );

    return {
      ...defaultMonth,
      rate: backendData?.percentage ?? defaultMonth.rate,
    };
  });
  ...
  useEffect(() => {
    ...
    const target = Number(overallPercentage ?? 0);
    const step = () => {
