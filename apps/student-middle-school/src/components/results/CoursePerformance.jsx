...
function CoursePerformance({ subjects = [] }) {
  const theme = useTheme();
  const isDarkMode = theme.palette.mode === 'dark';

  // Transform subjects data into test format
  const testsData = subjects.length > 0 ? [{
    id: "test1",
    title: "Latest Test Results",
    testData: subjects.map(subject => ({
      subject: subject.subject || subject.subjectName || "Subject",
      maxMarks: subject.maxMarks || 100,
      minMarks: subject.minMarks || 35,
      result: subject.marks || subject.result || 0,
      status: (subject.marks || subject.result || 0) >= (subject.minMarks || 35) ? "Pass" : "Fail"
    })),
    rank: subjects[0]?.rank || "N/A"
  }] : [];

  const [expanded, setExpanded] = useState(
    testsData.reduce((acc, test) => ({ ...acc, [test.id]: true }), {})
  );
  ...
  return (
    <Box id="coursePerformance" sx={{ fontFamily: "Poppins, sans-serif" }}>
      {testsData.length > 0 ? (
        ...
        >
          <Typography sx={{ color: isDarkMode ? "rgba(255,255,255,0.5)" : "#94A3B8", fontFamily: "Poppins, sans-serif" }}>
