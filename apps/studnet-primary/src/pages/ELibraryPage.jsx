...
function ELibraryPage() {
    ...
    const fetchLibraryData = async () => {
      try {
        ...
        const [booksData, newArrivalsData] = await Promise.all([
          getLibraryBooks('primary'),
          getLibraryNewArrivals('primary')
        ]);
  ...
  const renderBookCard = (book, index, keyPrefix) => (
    ...
    >
      ...
      <p className={`book-title text-sm sm:text-base font-bold mb-1 line-clamp-2 ${isDark ? 'text-[#FFFFFF]' : 'text-gray-800'}`} style={{ fontFamily: 'Poppins, sans-serif' }}>
        {book.title}
      </p>
      <p className={`book-author text-xs sm:text-sm ${isDark ? 'text-[#9CA3AF]' : 'text-gray-500'}`} style={{ fontFamily: 'Poppins, sans-serif' }}>
    ...
    return (
      <PageWithSidebar>
        <div className="flex flex-1 min-w-0 h-full p-2 sm:p-3 md:p-4 lg:p-6">
          <div className={`w-full h-full rounded-xl shadow-lg flex items-center justify-center ${isDark ? 'bg-[#0F1115]' : 'bg-slate-100'}`}>
            <div className="flex flex-col items-center gap-4">
              <Loader2 className="w-12 h-12 animate-spin text-blue-500" />
              <p className={`text-lg font-medium ${isDark ? 'text-[#FFFFFF]' : 'text-gray-600'}`}>Loading Library...</p>
            </div>
          </div>
        </div>
      </PageWithSidebar>
        ...
        <div className="flex flex-1 min-w-0 h-full p-2 sm:p-3 md:p-4 lg:p-6">
          <div className={`w-full h-full rounded-xl shadow-lg flex items-center justify-center ${isDark ? 'bg-[#0F1115]' : 'bg-slate-100'}`}>
            <div className="flex flex-col items-center gap-4 text-center p-8">
              ...
              <h2 className={`text-2xl font-bold ${isDark ? 'text-[#FFFFFF]' : 'text-gray-800'}`}>Error</h2>
              <p className={isDark ? 'text-[#9CA3AF]' : 'text-gray-600'}>{error}</p>
              <button
          ...
          `}>
            <div className="library-page">
              ...
              <section className="mb-6 sm:mb-8">
                <h2 className={`text-lg sm:text-xl font-bold mb-3 sm:mb-4 ${isDark ? 'text-[#FFFFFF]' : 'text-gray-800'}`} style={{ fontFamily: 'Poppins, sans-serif' }}>New uploads</h2>
                {newArrivals.length > 0 ? (
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4">
                    {newArrivals.map((book, index) => renderBookCard(book, index, 'new'))}
                  </div>
                ) : (
                  <p className={`text-center py-8 ${isDark ? 'text-[#9CA3AF]' : 'text-gray-500'}`}>No new arrivals available</p>
                )}
              </section>
              ...
              {/* All Books Section */}
              <section className="mb-6 sm:mb-8">
                <h2 className={`text-lg sm:text-xl font-bold mb-3 sm:mb-4 ${isDark ? 'text-[#FFFFFF]' : 'text-gray-800'}`} style={{ fontFamily: 'Poppins, sans-serif' }}>All</h2>
                {allBooks.length > 0 ? (
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4">
                    {allBooks.map((book, index) => renderBookCard(book, index, 'all'))}
                  </div>
                ) : (
                  <p className={`text-center py-8 ${isDark ? 'text-[#9CA3AF]' : 'text-gray-500'}`}>No books available</p>
                )}
              </section>
            </div>
          </div>
